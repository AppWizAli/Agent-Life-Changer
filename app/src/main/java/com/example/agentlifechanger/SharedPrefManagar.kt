...
fun putId(id:String)
{
    ...
    fun getProfit(): FaProfitModel {
        val json = sharedPref.getString("Profit", "") ?: ""

        // If the JSON string is empty or null, return a default InvestmentModel object
        if (json.isEmpty()) {
            return FaProfitModel() // Replace this with your default InvestmentModel constructor
        }

        // Try to deserialize the JSON string into an InvestmentModel object
        return try {
            Gson().fromJson(json, FaProfitModel::class.java)
        } catch (e: JsonSyntaxException) {
            // If the deserialization fails, return a default InvestmentModel object
            FaProfitModel() // Replace this with your default InvestmentModel constructor
        }
    }

    fun putFaProfit(faProfitModel: FaProfitModel) {
